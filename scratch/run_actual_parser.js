function parseImportedQuestions(rawText) {
  ...
  const isQuestionLike = (str) =>
    /[\؟\?]/.test(str) ||
    /(?:^|[^\p{L}\p{N}])(?:ما|ماذا|من|أين|اين|متى|كيف|لماذا|علل|فسر|اذكر|قارن|اختر|هل|كم|أي|اي|وضح|بين|صنف|عدد|اشرح|حدد|عرف|what|which|why|how|when|where|who|whose|whom)(?:[^\p{L}\p{N}]|$)/iu.test(str);
