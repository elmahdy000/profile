...
function parseImportedQuestions(rawText) {
  ...
  // Force newlines before question headers if missing
  cleanedText = cleanedText.replace(/([^\n])\s*(سؤال\s*\d+|س\d+|Question\s*\d+|#\d+)/gi, "$1\n$2");

  // Force newlines before choice prefixes if missing
  cleanedText = cleanedText.replace(/([^\n])\s+([A-Fa-fأابجده]|هـ|[1-6])\)\s+/g, "$1\n$2) ");

  // Force newlines before answers if missing
  cleanedText = cleanedText.replace(/([^\n])\s*(الإجابة(?:\s+الصحيحة)?|الاجابة(?:\s+الصحيحة)?|إجابة|اجابة|correct\s*answer|answer)\s*[:：\-]?\s*/gi, "$1\nالإجابة الصحيحة: ");

  // Force newlines before explanations if missing
...
async function main() {
  ...
  const parsed = parseImportedQuestions(rawResult.value);
  console.log(`Parsed Questions Count: ${parsed.questions.length}`);
  console.log(`Warnings Count: ${parsed.warnings.length}`);
  console.log("Sample Parsed Question #13:", JSON.stringify(parsed.questions[12], null, 2));
  console.log("Sample Parsed Question #25:", JSON.stringify(parsed.questions[parsed.questions.length - 1], null, 2));
}
