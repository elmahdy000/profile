...
export function getParentSession(req: any) {
  const token = req.cookies?.[PARENT_COOKIE];
  if (!token || typeof token !== "string") return null;
  return token;
}

// POST /api/parent/register (Register new parent linked to a student)
router.post("/parent/register", async (req, res, next) => {
  try {
    const { name, phone, studentIdentifier } = req.body;
    const parentName = String(name || "").trim();
    const parentPhone = String(phone || "").trim();
    const studentQuery = String(studentIdentifier || "").trim();
    ...
    // Find student by phone or access code
    const [student] = await db
      .select()
      .from(studentsTable)
      .where(
        and(
          ...
          sql`(${studentsTable.phone} = ${studentQuery} OR UPPER(${studentsTable.accessCode}) = UPPER(${studentQuery}))`
        )
      )
      .limit(1);

    if (!student) {
      res.status(404).json({ error: "لم نتمكن من العثور على طالب معتمد بهذا الرقم أو الكود. يُرجى التثبت من بيانات الطالب." });
      return;
    }
    ...
    if (existingParent) {
      res.json({
        success: true,
        message: "تم العثور على حسابك السابق لولي الأمر!",
        parentCode: existingParent.parentCode,
    ...
    const parentCode = `PAR-${Math.floor(100000 + Math.random() * 900000)}`;
    ...
    res.json({
      success: true,
      parentCode: newParent.parentCode,
      parentName: newParent.name,
      studentName: student.name,
      message: "تم تسجيل وتوثيق كود ولي الأمر بنجاح!",
    });
  } catch (error) {
    next(error);
  }
...
// POST /api/parent/login (Parent login via phone & parentCode)
router.post("/parent/login", async (req, res, next) => {
  try {
    const { phone, parentCode } = req.body;
    const pPhone = String(phone || "").trim();
    const pCode = String(parentCode || "").trim().toUpperCase();

    if (!pPhone || !pCode) {
      res.status(400).json({ error: "رقم هاتف ولي الأمر وكود التتبع مطلوبان" });
      return;
    }
    ...
    const [parent] = await db
      .select()
      .from(parentsTable)
      .where(and(eq(parentsTable.phone, pPhone), sql`UPPER(${parentsTable.parentCode}) = ${pCode}`))
      .limit(1);

    if (!parent) {
      res.status(401).json({ error: "بيانات الدخول غير صحيحة. يرجى التأكد من رقم الهاتف وكود ولي الأمر." });
      return;
    }
...
// GET /api/parent/me (Check parent auth & get student report)
router.get("/api/parent/report", async (req, res, next) => {
  try {
