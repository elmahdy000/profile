          ...
          <main className="min-w-0 lg:min-h-0 lg:overflow-y-auto">
            ...
            <div className="bg-background text-foreground dark:bg-slate-900 dark:text-white">
              <div className="flex flex-wrap items-center gap-2 border-b border-border p-3 sm:px-5">
                <button disabled={!previous} onClick={() => previous && onSelectLesson(previous)} className="inline-flex h-11 items-center gap-1 rounded-xl border border-border px-3 text-sm font-bold hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500 disabled:opacity-40"><ChevronRight className="h-4 w-4"/>السابق</button>
                <button disabled={!next} onClick={() => next && onSelectLesson(next)} className="inline-flex h-11 items-center gap-1 rounded-xl border border-border px-3 text-sm font-bold hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500 disabled:opacity-40">التالي<ChevronLeft className="h-4 w-4"/></button>
                <div className="mr-auto flex items-center gap-3"><span className="text-xs font-bold text-muted-foreground">{progress}% مكتمل</span><button disabled={progress >= 100 || saving} onClick={() => void markComplete()} className="inline-flex h-11 min-w-36 items-center justify-center gap-2 rounded-xl bg-sky-500 px-4 text-sm font-extrabold text-slate-950 hover:bg-sky-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-600 focus-visible:ring-offset-2 disabled:bg-emerald-100 disabled:text-emerald-700">{saving ? <Loader2 className="h-4 w-4 animate-spin"/> : <Check className="h-4 w-4"/>}{progress >= 100 ? "تم إكمال الدرس" : "تحديد كمكتمل"}</button></div>
              </div>

              <div className="border-b border-border px-3 sm:px-5"><div className="flex overflow-x-auto">{([['overview','نظرة عامة',FileText],['files','ملفات الدرس',Download],['notes','ملاحظاتي',StickyNote],['questions','الأسئلة',MessageCircle]] as const).map(([id,label,Icon]) => <button key={id} onClick={() => setTab(id)} className={`flex h-12 shrink-0 items-center gap-2 border-b-2 px-3 text-sm font-bold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-sky-500 ${tab===id ? "border-sky-500 text-sky-600 dark:text-sky-400" : "border-transparent text-muted-foreground hover:text-foreground"}`}><Icon className="h-4 w-4"/>{label}</button>)}</div></div>
              <div className="min-h-44 p-4 sm:p-6">
                {tab === "overview" && <div><h3 className="font-extrabold text-foreground">عن هذا الدرس</h3><p className="mt-2 max-w-3xl text-sm leading-7 text-muted-foreground">{item.description || "شاهد الدرس بالكامل، واستخدم الملفات والملاحظات لتثبيت المعلومات. يتم حفظ تقدمك تلقائيًا."}</p><div className="mt-4 flex flex-wrap gap-2 text-xs"><span className="rounded-lg bg-muted px-3 py-2 text-foreground">{item.category}</span>{item.stage && <span className="rounded-lg bg-muted px-3 py-2 text-foreground">{item.stage}</span>}</div></div>}
                {tab === "files" && <div className="grid gap-2 sm:grid-cols-2">{resources.length ? resources.map(file => <a key={file.id} href={`/api/learning/files/${file.id}/download`} className="flex min-h-14 items-center gap-3 rounded-xl border border-border p-3 hover:border-sky-300 hover:bg-sky-50 dark:hover:bg-sky-950/30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500"><FileText className="h-5 w-5 text-sky-600"/><span className="min-w-0 flex-1 truncate text-sm font-bold text-foreground">{file.title}</span><Download className="h-4 w-4 text-muted-foreground"/></a>) : <p className="text-sm text-muted-foreground">لا توجد ملفات مرفقة بهذا الدرس.</p>}</div>}
                {tab === "notes" && <div><div className="flex gap-2"><textarea value={noteText} onChange={event => setNoteText(event.target.value)} placeholder="اكتب ملاحظة مرتبطة بالوقت الحالي..." className="min-h-20 flex-1 resize-none rounded-xl border border-border bg-background p-3 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-sky-500"/><button onClick={addNote} disabled={!noteText.trim()} className="h-11 self-end rounded-xl bg-sky-500 px-4 text-sm font-bold text-white hover:bg-sky-400 disabled:opacity-40">حفظ</button></div><div className="mt-4 space-y-2">{notes.map(note => <button key={note.id} onClick={() => { if(videoRef.current){ videoRef.current.currentTime=note.at; void videoRef.current.play(); } }} className="flex w-full gap-3 rounded-xl border border-border p-3 text-right hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500"><span className="font-mono text-xs font-bold text-sky-600 dark:text-sky-400">{formatTime(note.at)}</span><span className="text-sm text-foreground">{note.text}</span></button>)}</div></div>}
                {tab === "questions" && <div>{quiz ? <button disabled={quiz.locked} onClick={() => onStartQuiz?.(quiz)} className="inline-flex h-11 items-center gap-2 rounded-xl bg-sky-500 px-5 text-sm font-bold text-white hover:bg-sky-400 disabled:cursor-not-allowed disabled:bg-muted disabled:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-600"><Captions className="h-4 w-4"/>{quiz.locked ? quiz.lockedReason || "أكمل الدرس أولًا" : "ابدأ اختبار الدرس"}</button> : <p className="text-sm text-muted-foreground">لا يوجد اختبار مرتبط بهذا الدرس حاليًا.</p>}</div>}
              </div>
            </div>
          </main>
