          ...
          <main className="min-w-0 lg:min-h-0 lg:overflow-y-auto">
            ...
            <div className="bg-white text-slate-900">
              ...
              <div className="min-h-44 p-4 sm:p-6">
                ...
                {tab === "questions" && <div>{quiz ? <button disabled={quiz.locked} onClick={() => onStartQuiz?.(quiz)} className="inline-flex h-11 items-center gap-2 rounded-xl bg-sky-500 px-5 text-sm font-bold hover:bg-sky-400 disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-600"><Captions className="h-4 w-4"/>{quiz.locked ? quiz.lockedReason || "أكمل الدرس أولًا" : "ابدأ اختبار الدرس"}</button> : <p className="text-sm text-slate-500">لا يوجد اختبار مرتبط بهذا الدرس حاليًا.</p>}</div>}
              </div>
            </div>
          </main>
