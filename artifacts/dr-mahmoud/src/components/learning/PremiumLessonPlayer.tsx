          ...
          <main className="min-w-0 flex-1 flex flex-col overflow-hidden">
            ...
            <div className="flex-1 min-h-0 overflow-y-auto bg-slate-900 text-slate-100 landscape:max-h-[500px]:hidden">
              ...
              <div className="min-h-44 p-4 sm:p-6">
                ...
                {tab === "files" && <div className="grid gap-2 sm:grid-cols-2">{resources.length ? resources.map(file => <a key={file.id} href={`/api/learning/files/${file.id}/download`} className="flex min-h-14 items-center gap-3 rounded-xl border border-white/10 bg-white/5 p-3 hover:border-sky-400/40 hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500"><FileText className="h-5 w-5 text-sky-400"/><span className="min-w-0 flex-1 truncate text-sm font-bold text-white">{file.title}</span><Download className="h-4 w-4 text-slate-400"/></a>) : <p className="text-sm text-slate-400">لا توجد ملفات مرفقة بهذا الدرس.</p>}</div>}
