...
type Student = { id:number; name:string; phone:string; email?:string|null; status:string; accessCode?:string|null; governorate?:string|null; city?:string|null; grade?:string|null; otherGradeDetail?:string|null; createdAt:string };
...
function authHeaders(json = true): HeadersInit {
  ...
  return { Authorization: `Bearer ${password}`, ...(json ? { "Content-Type": "application/json" } : {}) };
}

async function adminApi<T>(url:string, options:RequestInit={}):Promise<T>{
  const response=await fetch(url,{...options,headers:{...authHeaders(!(options.body instanceof FormData)),...(options.headers||{})}});
...
export function AdminLearning(){
  ...
  const load=async()=>{setLoading(true);try{const [s,f,q,a]=await Promise.all([adminApi<Student[]>("/api/admin/students"),adminApi<FileItem[]>("/api/admin/learning/files"),adminApi<Quiz[]>("/api/admin/learning/quizzes"),adminApi<Attempt[]>("/api/admin/learning/attempts")]);setStudents(s);setFiles(f);setQuizzes(q);setAttempts(a);}catch(e){toast({variant:"destructive",title:"خطأ",description:(e as Error).message});}finally{setLoading(false);}};
  useEffect(()=>{load();},[]);
