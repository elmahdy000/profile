  ...
  return (
    <>
      <div className="fixed inset-0 z-[80] flex items-center justify-center p-0 sm:p-4 lg:p-6">
        ...
        {/* Drawer Panel */}
        <div className=" relative z-10 flex h-full w-full flex-col overflow-hidden border border-slate-700/80 bg-card text-foreground shadow-[0_30px_100px_rgba(0,0,0,0.55)] sm:h-[calc(100vh-2rem)] sm:max-w-6xl sm:rounded-[28px] lg:h-[min(900px,calc(100vh-3rem))]">
          {/* Header */}
          <div className="flex items-center justify-between border-b border-slate-700/80 bg-card px-4 py-4 sm:px-7 sm:py-5">
            <div className="flex items-center gap-3">
              <div className="flex h-14 w-14 items-center justify-center rounded-2xl border border-[#1677FF]/30 bg-primary/15 text-xl font-black text-[#69A5FF]">
                {student.name.charAt(0)}
              </div>
              <div>
                <div className="mb-1 flex flex-wrap items-center gap-2">
                  <h2 className="text-lg font-extrabold text-foreground sm:text-xl">{student.name}</h2>
            ...
            <button
              ...
              className="grid h-11 w-11 place-items-center rounded-xl border border-slate-700 bg-slate-900/60 text-muted-foreground transition hover:bg-slate-800 hover:text-white"
            >
              <X className="h-5 w-5" />
            </button>
          </div>
          ...
          {/* Sub-Header Actions */}
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border bg-card/70 px-4 py-3 sm:px-7">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Button
                ...
                className="border-slate-700 bg-slate-800/80 text-xs font-bold text-foreground hover:bg-slate-700 h-8 rounded-xl"
              >
                <Send className="h-3.5 w-3.5 ml-1.5 text-primary" /> إشعار خاص
              </Button>
            ...
            <div className="relative">
              ...
              {showMoreActions && (
                <div className="absolute left-0 top-10 z-20 w-48 rounded-xl border border-slate-700 bg-card p-1.5 shadow-2xl space-y-1">
                  {student.status === "approved" && (
                    <button
                      type="button"
                      onClick={() =>
          ...
          {/* Navigation Tabs */}
          <div className="flex overflow-x-auto border-b border-border bg-[#0C1728] px-3 scrollbar-none sm:px-7">
            {[
              ...
              <button
                ...
                className={`whitespace-nowrap border-b-2 px-4 py-3.5 text-xs font-bold transition-colors ${
                  activeTab === tab.id
                    ? "border-[#1677FF] text-primary"
                    : "border-transparent text-muted-foreground hover:text-[#CBD5E1]"
                }`}
              >
                {tab.label}
              </button>
              ...
              <div className="grid gap-5 lg:grid-cols-[minmax(0,1.45fr)_minmax(280px,0.75fr)]">
                <section className="space-y-4">
                  ...
                  <div className="rounded-2xl border border-border bg-card p-4 space-y-1.5">
                    <span className="text-[11px] text-muted-foreground">المرحلة التعليمية</span>
                    <p className="text-sm font-extrabold text-foreground">{currentStudent.grade === "أخرى" ? currentStudent.otherGradeDetail || currentStudent.grade : currentStudent.grade || "غير محدد"}</p>
                  </div>
                  <div className="rounded-2xl border border-border bg-card p-4 space-y-1.5">
                    <span className="text-[11px] text-muted-foreground">اسم المدرسة</span>
                    <p className="text-xs font-extrabold text-foreground">{resolvedSchoolName || "غير محدد"}</p>
                  </div>
                  <div className="rounded-2xl border border-border bg-card p-4 space-y-1.5">
                    <span className="text-[11px] text-muted-foreground">الشعبة والمسار</span>
                    <p className="text-xs font-extrabold text-purple-300">{resolvedLanguageTrack || "عربي"}</p>
                  </div>
                  <div className="rounded-2xl border border-border bg-card p-4 space-y-1.5">
                    <span className="text-[11px] text-muted-foreground">تليفون ولي الأمر</span>
                    <p className="text-xs font-extrabold text-amber-300 dir-ltr text-right">{resolvedParentPhone || "غير مسجل"}</p>
                  </div>
                  <div className="rounded-2xl border border-border bg-card p-4 space-y-1.5">
                    <span className="text-[11px] text-muted-foreground">السنتر المختار</span>
                  ...
                  <div className="rounded-2xl border border-border bg-card p-4 space-y-1.5">
                    <span className="text-[11px] text-muted-foreground">الموعد المتاح</span>
                  ...
                  <div className="rounded-2xl border border-border bg-card p-4 space-y-1.5">
                    <span className="text-[11px] text-muted-foreground">نظام الحضور</span>
                    <p className="text-xs font-extrabold text-primary">
                      {currentStudent.learningMode === "offline" || resolvedCenterName ? "أوفلاين (السنتر)" : "أونلاين بالكامل"}
                    </p>
                  </div>
                  <div className="rounded-2xl border border-border bg-card p-4 space-y-1.5">
                    <span className="text-[11px] text-muted-foreground">المحافظة / المدينة</span>
                    <p className="text-xs font-extrabold text-foreground">
                      {currentStudent.governorate ? `${currentStudent.governorate} - ${currentStudent.city || ""}` : "الشرقية - الزقازيق"}
                    </p>
                  </div>
                  <div className="rounded-2xl border border-border bg-card p-4 space-y-1.5">
                    <span className="text-[11px] text-muted-foreground">تاريخ التسجيل</span>
                ...
                <aside className="space-y-4">
                  ...
                  <div className="overflow-hidden rounded-2xl border border-border bg-card">
                    {[
                      ...
                      <div key={label} className={`flex items-center justify-between gap-4 px-4 py-3.5 ${index ? "border-t border-border" : ""}`}>
                  ...
                  <button type="button" onClick={() => setActiveTab("courses")} className="flex w-full items-center justify-between rounded-2xl border border-[#1677FF]/30 bg-primary/10 p-4 text-right transition hover:bg-primary/15">
                    <span><strong className="block text-xs text-[#69A5FF]">إدارة كورسات الطالب</strong><small className="mt-1 block text-[10px] text-muted-foreground">عرض وتعديل صلاحيات الكورسات</small></span>
            ...
            {activeTab === "payments" && (
              <div className="space-y-4">
                <div className="flex flex-col gap-4 rounded-2xl border border-border bg-card p-5 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <span className="text-xs font-bold text-foreground">حالة الاشتراك الحالية</span>
                    <p className="text-xs text-muted-foreground">حدد ما إذا كان الطالب مشترك مدفوع أو مجاني.</p>
                  </div>
                  <select
                ...
                {receipt ? (
                  <div className="rounded-2xl border border-border bg-card p-5 space-y-4">
                    <h4 className="text-xs font-bold text-foreground">إيصال الدفع البنكي / المحفظة الإلكترونية</h4>
              ...
              <div className="space-y-4">
                <div className="rounded-2xl border border-border bg-card p-5 space-y-4">
                  <div className="flex items-center justify-between border-b border-border pb-3">
                    <div>
                      <h4 className="text-xs font-extrabold text-foreground">الكورسات المتاحة للطالب بالظبط</h4>
                      <p className="text-[11px] text-muted-foreground">حدد الكورسات المسموح بها بشكل خاص لهذا الطالب.</p>
                    </div>
                      ...
                      return (
                        <button
                          ...
                          className={`flex items-center justify-between rounded-xl px-3 py-2.5 text-xs font-bold border transition-all ${
                            isSelected
                              ? "border-[#1677FF] bg-primary/15 text-primary"
                              : "border-border bg-slate-900/60 text-[#CBD5E1] hover:border-slate-700"
                          }`}
                        >
                          <span className="dir-ltr text-right">{c.title}</span>
                          <span className={`h-4 w-4 rounded flex items-center justify-center border ${isSelected ? "border-[#1677FF] bg-primary text-white" : "border-slate-700"}`}>
            ...
            {activeTab === "security" && (
              <div className="space-y-3">
                <div className="rounded-xl border border-border bg-card p-4 flex items-center justify-between">
                  <div>
                    <h4 className="text-xs font-extrabold text-foreground">معرف الجهاز المقترن (Device Lock)</h4>
                    <p className="text-xs text-muted-foreground font-mono dir-ltr pt-1">
                  ...
                  <Button
                    ...
                    className="border-slate-700 bg-slate-800 text-xs font-bold text-foreground hover:bg-slate-700"
                  >
      ...
      {isEditingBooking && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/75 p-4 backdrop-blur-sm animate-in fade-in duration-150">
          <div className="w-full max-w-lg rounded-2xl border border-slate-700 bg-card p-6 text-right shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-border pb-3">
            ...
            <form onSubmit={handleSaveBooking} className="space-y-4 text-xs font-semibold">
              <div className="rounded-xl border border-border bg-card p-3 text-slate-300">
              ...
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  className="h-11 w-full rounded-xl border border-slate-700 bg-background px-3 text-white outline-none focus:border-blue-500"
                >
              ...
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  className="h-11 w-full rounded-xl border border-slate-700 bg-background px-3 text-white outline-none focus:border-blue-500 font-bold"
                >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    dir="ltr"
                    className="h-11 w-full rounded-xl border border-slate-700 bg-background px-3 text-white font-mono text-left outline-none focus:border-blue-500"
                  />
                </div>
              </div>
