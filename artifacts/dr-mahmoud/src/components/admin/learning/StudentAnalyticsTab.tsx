    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-slate-200 shadow-xs">
        <div>
          <div className="flex items-center gap-2">
            <h2 className="text-xl font-black text-slate-900">📊 تقرير نشاط الطلاب والمتابعة الدقيقة</h2>
        ...
        <button
          ...
          className="px-4 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold transition-all flex items-center justify-center gap-2 border border-slate-200 self-start md:self-auto"
        >
          <RefreshCw className={`w-4 h-4 ${isRefreshing ? "animate-spin text-[#0B63CE]" : ""}`} />
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            className="w-full pr-9 pl-4 py-2.5 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#0B63CE]"
          />
        </div>
        ...
        <div className="flex items-center gap-2 overflow-x-auto w-full md:w-auto pb-1 md:pb-0">
          <button
            ...
            className={`px-3.5 py-2 rounded-xl text-xs font-bold transition-all whitespace-nowrap ${
              filterType === "all" ? "bg-[#0B63CE] text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"
            }`}
          >
            الكل ({data.length})
          </button>
      ...
      {isLoading ? (
        <div className="bg-white p-12 rounded-2xl border border-slate-200 text-center space-y-3">
          <RefreshCw className="w-8 h-8 text-[#0B63CE] animate-spin mx-auto" />
          <p className="text-xs font-bold text-slate-600">جاري تحميل تقارير حركة الطلاب...</p>
        </div>
      ) : filteredData.length === 0 ? (
        <div className="bg-white p-12 rounded-2xl border border-slate-200 text-center space-y-2">
          <Users className="w-10 h-10 text-slate-300 mx-auto" />
          <h3 className="text-sm font-bold text-slate-800">لا يوجد طلاب يطابقون الفلتر الحالي</h3>
          <p className="text-xs text-slate-500">جرب البحث بكلمات أخرى أو تغيير نوع الفلتر</p>
        </div>
      ) : (
                    ...
                    <tr key={student.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="py-3.5 px-4">
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-xl bg-slate-100 text-slate-700 font-black flex items-center justify-center text-xs border border-slate-200">
                            {student.name.substring(0, 2)}
                          </div>
                          <div>
                            <strong className="block font-bold text-slate-900 text-xs">{student.name}</strong>
                      ...
                      {/* Last Login Time */}
                      <td className="py-3.5 px-4">
                        <span className="font-bold text-slate-800 block">{formatDate(student.lastLoginAt)}</span>
                      ...
                      <td className="py-3.5 px-4">
                        {latestWatch ? (
                          <div className="max-w-[200px]">
                            <span className="block font-bold text-slate-900 truncate text-[11px]" title={latestWatch.videoTitle}>
                      ...
                      <td className="py-3.5 px-4 text-center">
                        <span className="inline-block px-2.5 py-1 rounded-lg bg-blue-50 text-[#0B63CE] font-bold text-xs">
                      ...
                      <td className="py-3.5 px-4">
                        <div className="flex items-center justify-center gap-2">
                          <button
                            ...
                            className="px-3 py-1.5 rounded-xl bg-[#0B63CE] hover:bg-[#0952AC] text-white text-xs font-bold transition-all shadow-xs flex items-center gap-1"
                          >
      ...
      {selectedStudent && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white border border-slate-200 w-full max-w-3xl rounded-3xl p-6 relative z-10 max-h-[90vh] overflow-y-auto shadow-2xl space-y-6 text-right">
            
            {/* Modal Header */}
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-2xl bg-[#0B63CE]/10 text-[#0B63CE] font-black flex items-center justify-center text-base border border-[#0B63CE]/20">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-200/80">
                <span className="text-[10px] font-bold text-slate-500 block">حالة الحساب</span>
              ...
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-200/80">
                <span className="text-[10px] font-bold text-slate-500 block">تاريخ التسجيل</span>
                <span className="text-xs font-bold text-slate-800">{formatDate(selectedStudent.createdAt)}</span>
              </div>
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-200/80">
                <span className="text-[10px] font-bold text-slate-500 block">آخر دخول للنظام</span>
                <span className="text-xs font-bold text-slate-800">{formatDate(selectedStudent.lastLoginAt)}</span>
              </div>
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-200/80">
                <span className="text-[10px] font-bold text-slate-500 block">آخر نشاط ومشاهدة</span>
                <span className="text-xs font-bold text-slate-800">{formatDate(selectedStudent.lastActiveAt)}</span>
              </div>
            </div>
              ...
              {selectedStudent.watchDetails.length === 0 ? (
                ...
                <div className="space-y-2.5 max-h-60 overflow-y-auto pr-1">
                  {selectedStudent.watchDetails.map((watch, idx) => (
                    <div key={idx} className="p-3 bg-slate-50 rounded-xl border border-slate-200/80 flex items-center justify-between gap-3 text-xs">
                      <div className="min-w-0">
                        <strong className="block font-bold text-slate-900 truncate">{watch.videoTitle}</strong>
                        <div className="flex items-center gap-2 text-[10px] text-slate-500 mt-0.5">
                      ...
                      <div className="text-left flex-shrink-0">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] text-slate-500 font-mono">({formatSeconds(watch.currentTimeSeconds)})</span>
                        </div>
            ...
            <div className="space-y-3 pt-2">
              ...
              {selectedStudent.quizDetails.length === 0 ? (
                <div className="p-4 text-center bg-slate-50 rounded-2xl border border-slate-200 text-xs text-slate-500">
                ...
                <div className="space-y-2 max-h-40 overflow-y-auto pr-1">
                  {selectedStudent.quizDetails.map((quiz, idx) => (
                    <div key={idx} className="p-3 bg-slate-50 rounded-xl border border-slate-200/80 flex items-center justify-between text-xs">
                      <div>
                        <span className="font-bold text-slate-800">اختبار رقم #{quiz.quizId}</span>
                        <span className="text-[10px] text-slate-500 block">{formatDate(quiz.createdAt)}</span>
                      </div>
                      <div className="flex items-center gap-2">
