...
export function StudentsTab({
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Grade Quick Division Tabs */}
      <div className="flex items-center gap-1.5 overflow-x-auto pb-1 no-scrollbar border-b border-slate-200 pt-1">
        {[
          { id: "all", label: "جميع المراحل", count: students.length },
          { id: "bac2", label: "تانية بكالوريا", match: "تانية بكالوريا", count: bac2Count },
          { id: "bac1", label: "أولى بكالوريا", match: "أولى بكالوريا", count: bac1Count },
          { id: "uni", label: "المرحلة الجامعية", match: "المرحلة الجامعية", count: uniCount },
          { id: "sec1", label: "أولى ثانوي", match: "الأول الثانوي", count: students.filter((s) => s.grade && s.grade.includes("الأول الثانوي")).length },
        ].map((tabItem) => {
          const isActive =
            tabItem.id === "all"
              ? studentStageFilter === "all"
              : studentStageFilter.includes(tabItem.match || "");
          return (
            <button
              key={tabItem.id}
              type="button"
              onClick={() => {
                if (tabItem.id === "all") {
                  ...
                  const matchStage = studentStages.find((st) => st.includes(tabItem.match || "")) || "all";
                  setStudentStageFilter(matchStage);
                }
              }}
            ...
            >
              ...
              <span className={`rounded-full px-2 py-0.5 text-[10px] font-black ${isActive ? "bg-white/20 text-white" : "bg-slate-100 text-slate-600"}`}>
                {tabItem.count}
              </span>
            </button>
          );
